...
import com.sky.common.massage.Response;
...
public class HttpTookit {
    ...
    /**
     * 通过url post方式请求,返回json值并转换成对象返回. 
     * @param url     链接
     * @param params  map参数
     * @param t		      需要返回的对象
     * @return  T     返回的对象
     * @throws IOException 
     */
    public static <T> T  doPostJson(String url, Map<String, String> params,T t) throws IOException{
    	String values = postForm(url,params);
    	if(!StringUtil.isNull(values))
    		t = (T) JacksonUtil.fromJson(values, t.getClass());
    	return t;
    }
     ...
     * @return  T     返回的对象
     */
    public static <T> T  doGetJson(String url, Map<String, String> params,T t){
    	String urls="";
    	String valus="";
		try {
			urls = appendQueryParams(url,params);  //拼接url
			valus = get(url); //请求
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    	if(!StringUtil.isNull(valus))
    		t = (T) JacksonUtil.fromJson(valus, t.getClass());  //转化json
    	return t;
    }
...
//            String y = doGet("http://video.sina.com.cn/life/tips.html", null, "GBK", true); 
			...
			try {
				//post 方式测试
				rs = doPostJson("http://localhost:8080/api/login", map,rs);
				//get方式测试
				Map<String,String> map1 = new HashMap();
	    		map1.put("param", "1");
	    		map1.put("version", "1.0");
				rs = doGetJson("http://localhost:8080/api/user/get", map1,rs);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} 
